...
class BoxSizeClassifier(nn.Module):
    def __init__(self):
        ...
        self.classifier = nn.Sequential(nn.Flatten(), nn.Linear(1280, 128), nn.SiLU(), nn.Dropout(.2), nn.Linear(128, 4))
...
def warp(rgb, corners, capacity, cell_size):
...
class Chocolathon:
    def __init__(self, root, device='cpu', threads=4, batch_size=32, empty_mode='original', geometry='uniform', localizer='segmentation'):
        ...
        localization = self.root / 'data/experiments/box_localization_v2/deployment'
        ...
        expected_capacities = list(LAYOUTS)
        ...
        if set(map(int, routing)) != set(expected_capacities):
            raise ValueError('Localization corner routing must define 6, 16, 30 and 50.')
        ...
        if geometry == 'training-centers':
            ...
            for capacity, (rows, columns) in LAYOUTS.items():
                centers = []
    ...
    def smoke_test(self):
        ...
        if list(self.size_model(x).shape) != [1, 4]:
        ...
        return {
            ...
            'model_roles': {
                'box_size_classifier': 'predict 6/16/30/50 slot capacity',
                'tray_segmenter': 'primary automatic tray localization for all capacities',
                'corner_regressor_6_16': 'fallback tray corners for 6-slot and 16-slot boxes',
    ...
    def predict(self, image, annotation=None, grid=None, filled_only=False, capacity_override=None):
        ...
        if capacity_override is not None:
            ...
            if capacity_override not in LAYOUTS: raise ValueError('Capacity override must be 6, 16, 30 or 50.')
            probabilities = np.array([float(value == capacity_override) for value in LAYOUTS], np.float32)
            ...
            predicted_capacity = list(LAYOUTS)[int(probabilities.argmax())]
        ...
        tray = warp(rgb, corners, capacity, 256 if grid is not None or self.centers else 224)
        ...
        result = {
            ...
            'size_probabilities': dict(zip(map(str, LAYOUTS), map(float, probabilities))),
